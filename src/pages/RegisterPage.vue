<template>
  <q-page class="flex flex-center">
    ...
    <dialog-component v-model="showDialog"
    :propDialog="dialog"></dialog-component>
  </q-page>
</template>
...
export default defineComponent({
    ...
    methods: {
        ...
        usernameValidate() {
            if (this.username) {
                this.$api.get("/api/auth/" + this.username).then((response) => {
            ...
            if (this.upload_avatar) {
                ...
                this.$api.post("/api/file/upload", formData, {headers}).then((response)=>{
                  if(response.status == 200){
                    ...
                    // console.log("this is after upload img!!!"+ esponse.data.uploadFileName);
        ...
        submitData(filename){
          ...
          this.$api.post("/api/auth/signup", data).then((response)=>{
            if(response.status == 200){
              this.showDialog = true;
              this.dialog.icon = "task_alt";
              this.dialog.iconColor = "primary";
              this.dialog.msg = "<div class='text-h6'>Welcome, <br>"+
